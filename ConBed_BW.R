...
pint_dat <- data.table(copy(p_dat[idx_tbl$xid, 1:3, with = FALSE]), copy(w_dat[idx_tbl$yid, 1:3, with = FALSE]))
...
pint_dat <- copy(pint_dat[!1:nrow(pint_dat) %in% idx])
idx_tbl <- copy(idx_tbl[!1:nrow(idx_tbl) %in% idx])

tmp_pint <- paste0(tempfile(tmpdir = dout), ".bed")
dat_out <- copy(pint_dat[, 1:3, with = FALSE])
dat_out[, V2 := V2-1]
write.table(dat_out, file = tmp_pint, row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
...
mat_dat <- rbindlist(lapply(1:nrow(c_dat), function(i){
	...
	nd <- copy(dd[, sum(V1*w_width)/sum(w_width), by = yid])
